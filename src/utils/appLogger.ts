...
import { MessageRequest } from '../../src/hook/dto/messageRequest.dto';
...
export class AppLogger extends ConsoleLogger {
  logger = (data: DataLogger, context: string) => {
    ...
    if(requestData){
      this.log(JSON.stringify({ 
        ...
        requestData: (Object.keys(requestData).includes('WaId')) ? this.maskPersonalInfoReceived(deepCopy(requestData)) : requestData, 
        message 
        }), context);
  ...
  private mask = (info: string): string => '*'.repeat(info.length - 4) + info.substring(info.length - 4)
